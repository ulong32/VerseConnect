...
- **早期終了オプション**: すでに保存済みの画像が見つかった時点でインポートを終了するオプション（デフォルトONに相当する挙動をオフにできる）
- 進捗表示（プログレスバー）
- 重複ファイルの自動スキップ
- 200msの遅延によるサーバー負荷軽減
...
├── preload.cjs              # レンダラーにAPI公開 (Context Bridge)
├── main/
│   ├── index.js             # Electron メインプロセス エントリーポイント
│   ├── store.js             # electron-store 設定とデータ定義
│   ├── handlers/
│   │   └── aipriHandlers.js # 認証系・フォトインポート系 IPCハンドラー
│   └── services/
│       └── aipriService.js  # aipri.jp マイページとのセッション・通信・ダウンロード実処理
├── lib/
│   └── stores/
...
└── routes/
    ...
    └── import/
        └── +page.svelte      # インポートページ（アカウント管理 + ログイン + フォトインポートUI）
```
...
### Electron メインプロセス構成 (`src/main/`)

認証系やフォト取得の仕組みは、レンダラーからのIPC通信を受け取るハンドラー層（`src/main/handlers/aipriHandlers.js`）と、実際のマイページとのHTTP通信やダウンロードを行うサービス層（`src/main/services/aipriService.js`）に分離されています。

#### 主なインポート

```javascript
// aipriService.js / aipriHandlers.js
...
#### ストア拡張 (`src/main/store.js`)

`electron-store` に multi-account 用フィールドを追加し、永続化しています。
...
#### Cookie取得の仕組み (`aipriService.js`)

`redirect: 'follow'` を使用する場合、リダイレクト時のSet-Cookieヘッダーは最終レスポンスで取得できないため、ElectronのCookieストア `session.defaultSession.cookies.get()` を使用してCookieを取得・検証します。
...
| -------------------- | -------------------------------------------------------------------- |
| 対象月セレクト       | 今月/先月を選択                                                      |
| すべての画像を確認   | オフの場合、既存の画像が見つかった時点で停止する（デフォルト: オフ） |
...
3. 各フォトに対して `aipri-download-photo` を実行
   ...
   - 既存ファイルはスキップ
   - **早期終了**: 「すべての画像を確認する」がオフで、既存ファイルが見つかった場合はその時点でループを終了
   - 200ms間隔でダウンロード（サーバー負荷軽減）
4. 結果メッセージを表示
   - 成功: 「X枚をインポートしました」
   - 早期終了: 「既存の画像が見つかったためインポートを終了しました」
...
| 2026-05-19 | 既存画像発見時の早期終了オプションを追加                               |
