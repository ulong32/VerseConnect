<script lang="ts">
  ...
  let subFolder = $state("");
  let verifyAllImages = $state(false);
  ...
  async function handleImport() {
    ...
    if (!baseFolderPath) {
      importResult = {
        success: false,
        message:
          "フォルダが設定されていません。設定画面で保存先フォルダを選択してください。",
      };
      return;
    }
    ...
    try {
      ...
      const fetchResult = (await window.electronAPI.aipriFetchPhotos(
        selectedYm,
      )) as AipriFetchPhotosResult & { reloggedIn?: boolean };
      ...
      if (!fetchResult.success || !fetchResult.photos) {
        importResult = {
          success: false,
          message: fetchResult.error || "フォトリストの取得に失敗しました",
        };
        isImporting = false;
        return;
      }
      ...
      for (const photo of photos) {
        ...
        if (result.success) {
          if (result.skipped) {
            skippedCount++;
            // Stop importing if we hit an existing photo and verifyAllImages is false
            if (!verifyAllImages) {
      ...
      const totalSkipped = photos.length - downloadedCount - errorCount;
      const stats = `${downloadedCount}枚をインポート、${totalSkipped}枚をスキップ`;

      if (
        downloadedCount === 0 &&
        totalSkipped > 0 &&
        !verifyAllImages &&
        importProgress.current < photos.length
      ) {
        importResult = {
          success: true,
          message: `既存の画像が見つかったため終了しました（${stats}）`,
        };
      } else if (downloadedCount === 0 && totalSkipped === photos.length) {
        importResult = {
          success: true,
          message: `すべて既存のためスキップしました（${totalSkipped}枚）`,
        };
      } else if (errorCount > 0) {
        importResult = {
          success: false,
          message: `${stats}、${errorCount}枚がエラー`,
        };
      } else {
        importResult = {
          success: true,
      ...
      importResult = {
        success: false,
        message: `エラーが発生しました: ${String(error)}`,
      };
    } finally {
      isImporting = false;
    }
  ...
  <div class="max-w-2xl mx-auto">
    ...
    {:else if sessionState.isLoggedIn}
      ...
      <section class="mb-8">
        <div class="bg-white/5 rounded-xl p-6 backdrop-blur-sm">
          ...
          <div class="border-t border-white/10 pt-6 mt-6">
            <h3
              class="text-md font-semibold text-white mb-4 flex items-center gap-2"
            >
              ...
              <div class="flex items-center justify-between">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-gray-400"
                    >オフの場合、既存の画像が見つかった時点で停止します</span
                  >
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    bind:checked={verifyAllImages}
                    class="sr-only peer"
                    disabled={isImporting}
                  />
                  <div
                    class="w-11 h-6 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:bg-purple-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all"
                  ></div>
                </label>
              </div>
            </div>
              ...
              <div class="bg-white/10 rounded-lg p-4 mb-4">
                ...
                >
                  <div class="flex justify-between text-sm text-gray-300">
                    <Progress.Label
                      >ダウンロード中... {importProgress.current} / {importProgress.total}</Progress.Label
                    >
                    {#if importProgress.skipped > 0}
                      <span class="text-gray-400"
                        >({importProgress.skipped}件スキップ)</span
                      >
                    {/if}
                  </div>
                  <Progress.Track
                    class="bg-white/20 h-2 rounded-full overflow-hidden"
                  >
                    <Progress.Range
                      class="bg-linear-to-r from-emerald-400 to-teal-500 h-full transition-all duration-300"
                    />
                  </Progress.Track>
                </Progress>
              </div>
            ...
            {#if importResult}
              <div
                class="p-3 rounded-lg {importResult.success
      ...
      {#if showAddForm}
        ...
        >
          <div
            class="bg-[#1e1e2e] rounded-xl p-6 w-full max-w-md shadow-2xl border border-white/10"
          >
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  id="addCardId"
                  class="w-full px-3 py-2 bg-white/10 border rounded-lg text-white placeholder:text-gray-500 focus:outline-none transition-colors {cardIdTouched &&
                  cardIdError
              ...
              <div>
                ...
                <input
                  type="text"
                  id="addName"
                  class="w-full px-3 py-2 bg-white/10 border rounded-lg text-white placeholder:text-gray-500 focus:outline-none transition-colors {nameTouched &&
                  nameError
                    ? 'border-red-500 focus:border-red-500'
                    : 'border-white/20 focus:border-purple-500'}"
                  placeholder="10文字まで"
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <input
                    ...
                    id="addBirthdayM"
                    inputmode="numeric"
                    class="w-20 px-3 py-2 bg-white/10 border rounded-lg text-white placeholder:text-gray-500 focus:outline-none transition-colors text-center {birthdayMTouched &&
                    birthdayMError
                      ? 'border-red-500 focus:border-red-500'
                      : 'border-white/20 focus:border-purple-500'}"
                    placeholder="01"
                  ...
                  <input
                    ...
                    id="addBirthdayD"
                    inputmode="numeric"
                    class="w-20 px-3 py-2 bg-white/10 border rounded-lg text-white placeholder:text-gray-500 focus:outline-none transition-colors text-center {birthdayDTouched &&
                    birthdayDError
                      ? 'border-red-500 focus:border-red-500'
                      : 'border-white/20 focus:border-purple-500'}"
                    placeholder="01"
              ...
              {#if sessionState.error}
                <div
                  class="p-3 bg-red-500/20 border border-red-500/50 rounded-lg"
                >
