<script lang="ts">
  ...
  // Metadata state
  let currentMetadata: ImageMetadata = $state({
    characters: [],
    item: "",
    tags: [],
  });
  ...
  let filteredImages = $derived.by(() => {
    ...
    if (selectedMonth) {
      result = result.filter((img) =>
        img.extractedDate?.startsWith(selectedMonth),
      );
    }
    ...
    if (noCharacterFilter) {
      result = result.filter(
        (img) =>
          !img.metadata?.characters || img.metadata.characters.length === 0,
      );
    }
    ...
    if (noItemFilter) {
      result = result.filter(
        (img) => !img.metadata?.item || img.metadata.item.trim() === "",
      );
    }
    ...
    if (noTagFilter) {
      result = result.filter(
        (img) => !img.metadata?.tags || img.metadata.tags.length === 0,
      );
    }
  ...
  function prevImage() {
    if (selectedIndex > 0) {
      ...
      currentMetadata = selectedImage.metadata || {
        characters: [],
        item: "",
        tags: [],
      };
  ...
  function toggleCharacter(char: string) {
    if (currentMetadata.characters.includes(char)) {
      currentMetadata.characters = currentMetadata.characters.filter(
        (c) => c !== char,
      );
    } else {
  ...
  async function selectFriendCard(file: File) {
    if (!window.electronAPI || !settingsState.folderPath || !selectedImage)
      return;
    ...
    reader.onload = async () => {
      ...
      const result = await window.electronAPI.saveFriendCard(
  ...
  async function saveMetadata() {
    ...
    const targetFolder = selectedImage.folder
      ? `${settingsState.folderPath}/${selectedImage.folder}`.replace(
          /\\/g,
          "/",
        )
      : settingsState.folderPath;

    await window.electronAPI.setImageMetadata(
      targetFolder,
      selectedImage.name,
      metadataToSave,
    );
  ...
  async function applyBulkCharacters(
    characters: string[],
...
>
    ...
    <div class="flex items-center justify-between mb-2 px-2">
      <div class="flex items-center gap-3">
        <span class="text-gray-400 text-sm"
          >{filteredImages.length}件のプリフォト</span
        >
        <button
      ...
      <button
        class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-colors {showTileInfo
  ...
  <!-- Image Grid -->
  <div
    class="flex-1 grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] auto-rows-auto gap-3 p-2 content-start"
    in:fly={{ duration: 500, y: "10vh", opacity: 0 }}
  >
    {#if isLoading}
      <div
        class="col-span-full flex items-center justify-center h-48 text-gray-500 text-xl"
      >
        読み込み中...
      </div>
      ...
      {#each displayedImages as image, index}
        ...
        >
          {#if isMultiSelectMode}
            ...
            >
              <div
                class="absolute top-2 left-2 z-10 size-6 rounded border-2 flex items-center justify-center transition-colors {selectedImages.has(
                  image.path,
                )
      ...
      {#if displayCount < filteredImages.length}
        ...
        >
          <div class="flex items-center gap-2">
            <div
              class="animate-spin size-5 border-2 border-purple-500 border-t-transparent rounded-full"
            ></div>
