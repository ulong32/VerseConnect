<script lang="ts">
	import { CalendarIcon, HashIcon, FolderClosedIcon, TagIcon, UserRoundIcon, ImageIcon } from '@lucide/svelte';
	interface Props {
		image: ImageInfo;
		onclick: () => void;
	}
...
>
	...
	<div class="w-full overflow-hidden bg-gray-900 aspect-square">
		<img class="w-full h-full object-cover" src={image.url} alt={image.name} loading="lazy" />
	</div>
	...
	<div class="w-full bg-black/80 p-2">
		...
		<div class="flex items-center gap-2 text-xs mb-1 text-gray-400">
			<span class="flex items-center gap-1">
				...
				<span class="truncate max-w-[60px]">{image.folder || '.'}</span>
			</span>
		</div>
		<!-- Item -->
		<div class="flex items-center gap-2 text-xs mb-1">
			...
			{#if image.metadata?.item}
				<img 
					class="size-16 object-contain rounded flex-shrink-0" 
			...
			{:else}
				<div class="size-16 bg-gray-700/50 rounded flex items-center justify-center flex-shrink-0">
		...
		<div class="flex items-start gap-1 text-xs">
			...
			<div class="flex flex-wrap gap-1 min-h-[20px]">
				{#if image.metadata?.characters && image.metadata.characters.length > 0}
					{#each image.metadata.characters as char}
	...
	<div class="absolute top-0 left-0 right-0 p-2 bg-gradient-to-b from-black/80 to-transparent text-white text-xs truncate opacity-0 group-hover:opacity-100 transition-opacity">
		{image.name}
	</div>
</button>
